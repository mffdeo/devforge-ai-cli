...
def _commands_panel() -> Panel:
    ...
    rows = [
        ...
        ("3", f"[bold {t.TEXT}]profile approve[/bold {t.TEXT}]", "Aprova perfil do projeto"),
        ("4", f"[bold {t.TEXT}]specify[/bold {t.TEXT}]",       "Transforma ideia em SPEC"),
        ("5", f"[bold {t.TEXT}]plan[/bold {t.TEXT}]",          "Gera plano com spec"),
        ("6", f"[bold {t.TEXT}]implement[/bold {t.TEXT}]",     "Chama agente externo"),
        ("7", f"[bold {t.TEXT}]policy check[/bold {t.TEXT}]",  "Avalia política de conformidade"),
        ("8", f"[bold {t.TEXT}]review[/bold {t.TEXT}]",        "Registra revisão humana"),
        ("9", f"[bold {t.TEXT}]evidence[/bold {t.TEXT}]",      "Coleta e reporta evidências"),
        ("10", f"[bold {t.TEXT}]pr-ready[/bold {t.TEXT}]",      "Prepara commit e PR"),
    ]
